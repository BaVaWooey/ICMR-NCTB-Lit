...
class CategoricFluencyFruit extends StatefulWidget {
  const CategoricFluencyFruit({super.key});

  @override
  State<CategoricFluencyFruit> createState() => _CategoricFluencyFruitState();
}

class _CategoricFluencyFruitState extends State<CategoricFluencyFruit> {
  ...
  void changeTimerValue() async {
    if (categoricFluencyRecordingInProgress == true ||
        categoricFluencyFruitComplete == true) {
      return;
    }
    while (timerValue > 0) {
  ...
  void recordAndPost() async {
    ...
    var newDirectory = await Directory('$path/fruit').create(recursive: true);
    debugPrint(newDirectory.path);
    ...
    ///testing if the text to speech has worked accurately.
    ////
    respStr = replaceOutputFunction(respStr);

    ///
    ///we 'unset' the recording variable and set the output values to a single list of words
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          // title: const Text('Categoric Fluency Fruit',
          //     style: TextStyle(fontSize: 40)),
          title: const Row(mainAxisSize: MainAxisSize.min, children: [
            Text('Categoric Fluency Fruit', style: TextStyle(fontSize: 40)),
            Icon(Icons.apple_sharp, size: 50, color: Colors.red)
          ]),
          centerTitle: true,
          toolbarHeight: 100),
      ...
      body: Column(children: [
        ...
        Padding(
            ...
            child: MaterialButton(
                ...
                color: Colors.blue,
                child: categoricFluencyFruitComplete == false
                    ? SizedBox(
                        height: 60,
                        width: 60,
        ...
        Padding(
            ...
            child: categoricFluencyFruitComplete == false
                ...
                : Text(categoricFluencyCompletedFruit,
                    style: const TextStyle(
                        fontSize: 30,
                        height: 1.5))) // add variable property here
      ]),
      floatingActionButton: Visibility(
          visible: categoricFluencyFruitComplete &&
                  !categoricFluencyRecordingInProgress
              ? true
