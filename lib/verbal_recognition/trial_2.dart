...
class VerbalMemoryTrialSecond extends StatefulWidget {
  const VerbalMemoryTrialSecond({super.key});

  @override
  State<VerbalMemoryTrialSecond> createState() =>
      _VerbalMemoryTrialSecondState();
}

class _VerbalMemoryTrialSecondState extends State<VerbalMemoryTrialSecond> {
  ...
  void startRecording() async {
    ...
    var trial1Directory =
        await Directory('$path/verbal_memory/trial_2').create(recursive: true);
  ...
  void stateWords() async {
    ...
    while (count < 10) {
      audioWord = wordToAudio[allRecitals[1][count]]!;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
            title: const Text("Trial 2", style: TextStyle(fontSize: 40)),
            toolbarHeight: 100,
            centerTitle: true),
        body: startTimer > 0
            ? Center(
        ...
        floatingActionButton: Visibility(
            ...
            child: FloatingActionButton.extended(
                onPressed: () => {
                      setState(() => trialNumberImmediate = 3),
                      Navigator.push(
                          context,
                          MaterialPageRoute(
